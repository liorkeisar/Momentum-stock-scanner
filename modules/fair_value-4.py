...
from modules.data_sources import load_history, fetch_analyst_data
...
# ---------------------------------------------------------------
# שיפורים: קצב צמיחה רב-שנתי, יעד קונצנזוס אנליסטים, השוואת סקטור
# ---------------------------------------------------------------

@st.cache_data(ttl=3600, show_spinner=False)
def fetch_multi_year_eps_growth(ticker):
    """
    מחשב CAGR (קצב צמיחה שנתי מורכב) של הרווח למניה על פני כמה שנים אחרונות
    (בדרך כלל 3-4, לפי מה שYahoo חושף), במקום להסתמך על שינוי שנה אחת בלבד
    (earningsGrowth) שיכול להיות מוטה משמעותית ע"י בסיס נמוך חד-פעמי (למשל
    התאוששות מהפסד/רווח חלש בשנה קודמת). זו בדיוק הבעיה שגילינו קודם עם
    סיטיגרופ. מחזיר אחוז (למשל 12.5) או None אם אין מספיק נתונים אמינים.
    """
    try:
        t = yf.Ticker(ticker)
        income = None
        ...
        for row_name in ["Diluted EPS", "Basic EPS"]:
            if row_name in income.index:
                eps_row = income.loc[row_name].dropna()
                break
        if eps_row is None or len(eps_row) < 2:
            return None

        eps_row = eps_row.sort_index()  # יישור כרונולוגי: ישן -> חדש
        eps_oldest, eps_newest = eps_row.iloc[0], eps_row.iloc[-1]
        years = len(eps_row) - 1
        if is_bad(eps_oldest) or is_bad(eps_newest) or eps_oldest <= 0 or eps_newest <= 0 or years <= 0:
            return None

        cagr = (eps_newest / eps_oldest) ** (1.0 / years) - 1.0
        return cagr * 100
    except Exception:
        return None


def compute_analyst_consensus_valuation(ticker):
    """
    שיטה שישית: יעד המחיר הממוצע (mean) של אנליסטים כפי שמדווח ב-Yahoo Finance -
    לא מודל תיאורטי אלא קונצנזוס אמיתי של גורמים מקצועיים שעוקבים אחרי החברה.
    """
    try:
        analyst = fetch_analyst_data(ticker)
        targets = analyst.get("targets") if analyst else None
        if not targets or is_bad(targets.get("mean")) or targets["mean"] <= 0:
            return None
        return {"fair_price": float(targets["mean"]), "low": targets.get("low"), "high": targets.get("high")}
    except Exception:
        return None
...
SECTOR_PEER_MAP = {
    ...
    "Basic Materials": ["LIN", "SHW", "APD"],
}


@st.cache_data(ttl=21600, show_spinner=False)
def fetch_sector_avg_pe(sector):
    """
    ממוצע P/E של כמה חברות ענק מייצגות (peers) מאותו סקטור - קירוב ל"P/E ממוצע
    סקטוריאלי" (Yahoo/yfinance לא חושפים ישירות מדד כזה). Cache ל-6 שעות כי
    זה לא משתנה מהר. אם הסקטור לא מוכר לנו, או אין נתונים - מחזיר None בשקט.
    """
...
def compute_sector_relative_valuation(eps, sector):
    """שיטה נוספת: P/E ממוצע הסקטור (קירוב מ-peers מובילים) × EPS."""
    try:
        avg_pe = fetch_sector_avg_pe(sector)
        if avg_pe is None or is_bad(eps) or eps <= 0:
            return None
        fair_price = avg_pe * eps
        return {"fair_price": fair_price, "sector_avg_pe": avg_pe}
    except Exception:
        return None
...
def compute_fair_value_report(ticker, df_price=None, growth_rate_override=None,
    ...
    """מריץ את כל שיטות השווי ההוגן הזמינות (עד 7) ומחזיר דוח מרוכז עם ממוצע משוקלל, חציון, ואזהרת פיזור."""
    ...
    if growth_pct is None:
        # מעדיפים CAGR רב-שנתי (יציב יותר) על פני שינוי שנה אחת בלבד -
        # בדיוק מה שמנע קודם את ההערכה המנופחת שקיבלנו לסיטיגרופ.
        multi_year_g = fetch_multi_year_eps_growth(ticker)
        if not is_bad(multi_year_g):
            growth_pct = multi_year_g
        else:
    ...
    if graham:
        methods["Graham מתוקן (EPS+צמיחה+תשואת אג\"ח)"] = {"fair_price": graham, "weight": 0.14}

    if df_price is not None and not df_price.empty:
        pe_rel = compute_pe_relative(df_price, eps, current_pe)
        if pe_rel:
            methods["P/E יחסי-היסטורי (IQR)"] = {"fair_price": pe_rel["fair_price"], "weight": 0.10}
    ...
    if peg:
        methods["PEGY (כלל פיטר לינץ')"] = {"fair_price": peg["fair_price"], "weight": 0.10}

    pb = compute_justified_pb(fund.get("returnOnEquity"), fund.get("payoutRatio"), cost_of_equity, bvps)
    if pb:
        methods["P/B מוצדק (CAPM · Damodaran)"] = {"fair_price": pb["fair_price"], "weight": 0.14}
    ...
    if dcf:
        methods["DCF דו-שלבי (WACC)"] = {"fair_price": dcf["fair_price"], "weight": 0.24}

    analyst_val = compute_analyst_consensus_valuation(ticker)
    if analyst_val:
        methods["יעד קונצנזוס אנליסטים"] = {"fair_price": analyst_val["fair_price"], "weight": 0.18}

    sector = fund.get("sector")
    sector_val = compute_sector_relative_valuation(eps, sector)
    if sector_val:
        methods["P/E יחסי-סקטור"] = {"fair_price": sector_val["fair_price"], "weight": 0.10}
...
def render_fair_value_screen():
    """מסך שווי הוגן נפרד — עד 7 שיטות (Graham מתוקן, P/E יחסי, PEGY, P/B מוצדק-CAPM, DCF-WACC, קונצנזוס אנליסטים, יחסי-סקטור)."""
    st.subheader("💰 מסך שווי הוגן (Fair Value)")
    st.caption(
        "הערכת שווי הוגן משולבת מעד 7 מקורות: 5 מודלים פיננסיים קלאסיים (Graham המתוקן, P/E יחסי-היסטורי, "
        "PEGY (פיטר לינץ'), P/B מוצדק לפי CAPM (Damodaran), ו-DCF דו-שלבי מהוון ב-WACC) - "
        "ובנוסף יעד קונצנזוס אנליסטים אמיתי ו-P/E יחסי מול ממוצע הסקטור (peers מובילים), כאשר זמינים. "
    ...
    t = get_theme()
    if not is_bad(w_fp) and not is_bad(avg_up):
        ...
        arrow_html = f"""
        <div style="background:{t['panel']}; border:1px solid {t['border']}; border-radius:16px; padding:18px 16px; margin-bottom:14px;">
            <div style="display:flex; justify-content:space-around; align-items:center; text-align:center;">
                <div>
                    <div style="font-size:12px; color:{t['text_muted']};">מחיר נוכחי</div>
                    <div style="font-size:26px; font-weight:800; color:{t['text_main']};">${cur_price:,.2f}</div>
                </div>
                <div style="font-size:24px; color:{ACCENT};">←</div>
                <div>
                    <div style="font-size:12px; color:{t['text_muted']};">שווי הוגן משוקלל</div>
                    <div style="font-size:26px; font-weight:800; color:{badge_color};">${w_fp:,.2f}</div>
                </div>
            </div>
            <div style="text-align:center; margin-top:14px;">
                <span class="score-badge" style="background:{badge_color}22; color:{badge_color}; border:1px solid {badge_color}55; font-size:15px;">{badge_text}</span>
            </div>
            <div style="text-align:center; margin-top:8px; font-size:12px; color:{t['text_muted']};">
                שווי הוגן — חציון: ${med_fp:,.2f}
            </div>
        </div>
        """
        st.markdown("".join(line.strip() for line in arrow_html.split("\n")), unsafe_allow_html=True)
    else:
