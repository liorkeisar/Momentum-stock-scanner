...
def render_css():
    ...
    <style>
        ...
        /* ---------- כרטיס מניה מצומצם (v3) - בהשראת רפרנס עם פילים וגרף עמודות ---------- */
        .stock-card-v3 {{
            background: {t['panel']}; border: 1px solid {t['border']}; border-radius: 18px;
            padding: 14px 16px; margin-bottom: 12px;
            box-shadow: {t['shadow']};
        }}
        .v3-ticker-row {{ display: flex; align-items: center; justify-content: space-between; gap: 8px; }}
        .v3-ticker {{ font-size: 20px; font-weight: 800; color: {t['text_main']}; }}
        .v3-price-row {{ display: flex; align-items: baseline; gap: 8px; margin-top: 2px; }}
        .v3-price {{ font-size: 24px; font-weight: 800; color: {t['text_main']}; }}
        .v3-chg {{ font-size: 13px; font-weight: 700; }}
        .v3-bars {{ margin: 8px 0 6px 0; height: 44px; }}
        .v3-bars svg {{ width: 100%; height: 100%; display: block; }}
        .v3-mid-row {{ display: flex; align-items: center; justify-content: space-between; gap: 8px;
                       margin-top: 4px; flex-wrap: wrap; }}
        .v3-score {{ font-size: 13px; font-weight: 700; color: {t['text_muted']}; white-space: nowrap; }}

        /* פילים (badges) בסגנון כדוריות מלאות-צבע, כמו ברפרנס */
        .pill {{
            display: inline-block; padding: 4px 12px; border-radius: 30px;
            font-size: 12px; font-weight: 800; white-space: nowrap;
        }}
        .pill-green {{ background: {BUY_COLOR}; color: #06210f; }}
        .pill-red {{ background: {SELL_COLOR}; color: #2b0505; }}
        .pill-navy {{ background: {t['panel_alt']}; color: {t['text_secondary']}; border: 1px solid {t['border']}; }}
        .pill-neutral {{ background: {t['border']}; color: {t['text_muted']}; }}
        ...
        /* ---------- סליידרים בסגנון "קלף" (בהשראת אפליקציות מובייל מודרניות) ---------- */
        /* ⚠️ הערת תחזוקה: הסלקטורים כאן (data-testid/data-baseweb) מבוססים על מבנה ה-DOM
           הנוכחי של רכיב הסליידר ב-Streamlit. אם גרסת Streamlit תשתנה משמעותית, ייתכן
           שחלק מהעיצוב "ייפול" - אם זה קורה, צריך לבדוק את המבנה בפועל ולעדכן כאן. */
        ...
        /* פס המילוי (בין שתי הידיות בסליידר טווח, או מ-0 עד הידית בסליידר בודד) */
        ...
        div[data-baseweb="slider"] div[role="slider"] {{
            ...
            border: 3px solid {t['panel']} !important;
            box-shadow: 0 0 0 1px {ACCENT};
        }}
        /* בועת הערך שמופיעה מעל הידית - הפכנו לגמרי ל-DEFAULT של Streamlit (בלי שום
           עיצוב מותאם אישית) אחרי שניסיונות עיצוב קודמים (הגדלת פונט, אז רקע+נו-ראפ)
           גרמו פעמיים לחפיפה/הדבקה של שני ערכים בסליידר טווח (כמו "65"+"95"->"6595").
           זו לא נוסחה שאני יכול להמשיך "לנחש" בלי לראות את ה-DOM בפועל - עדיף
           לוותר על העיצוב כאן מאשר לשבור קריאות. */

        /* תוויות מינימום/מקסימום מתחת לפס */
        div[data-testid="stSliderTickBarMin"], div[data-testid="stSliderTickBarMax"] {{
            color: {t['text_muted']} !important;
            font-size: 11.5px !important;
        }}

        /* ---------- ליטוש נוסף לכרטיסים/תגיות - בהשראת אפליקציות מובייל מודרניות ---------- */
        .tag {{
            font-weight: 800 !important;
            letter-spacing: 0.2px;
        }}
        .tag-buy {{ box-shadow: 0 2px 8px rgba(34,197,94,0.15); }}
        .tag-sell {{ box-shadow: 0 2px 8px rgba(239,68,68,0.15); }}
        .stock-card-v2 {{ transition: box-shadow 0.15s ease-in-out; }}
        .score-ring-big-inner {{ box-shadow: inset 0 0 0 1px {t['border']}; }}
        .stat-box, .stat-row-v2 .item {{ transition: background 0.15s ease-in-out; }}
    </style>
...
def render_top_nav():
    """
    ניווט ראשי בסגנון שבבים אופקיים (pills), מיד מתחת לכותרת/באנר.

    נבחר בכוונה עם st.radio(horizontal=True) ולא st.columns של כפתורים:
    Streamlit מפרק אוטומטית columns לעמודה אנכית אחת במסכים צרים (בדיוק
    המצב במובייל) - זה מה ששבר את סרגל הניווט התחתון בגרסה הקודמת (הוא
    הופיע כרשימה אנכית פשוטה במקום שורת ניווט). st.radio(horizontal=True)
    לעומת זאת מתנהג כווידג'ט יחיד עם flex-wrap (ראה CSS למעלה על
    div[data-testid="stRadio"]) שכבר מוכח כעובד היטב במובייל באפליקציה הזו
    (בדיוק אותו רכיב משמש כבר ל"מיין לפי"/"תצוגה" בטאב הסורק).
    """
    active = st.session_state.get("active_section", "scanner")
    keys = [s[0] for s in NAV_SECTIONS]
    labels = {k: f"{icon} {label}" for k, icon, label in NAV_SECTIONS}
    idx = keys.index(active) if active in keys else 0
    choice = st.radio(
        "ניווט", options=keys, format_func=lambda k: labels[k],
        index=idx, horizontal=True, label_visibility="collapsed", key="top_nav_radio",
    )
    if choice != active:
...
def render_stepper(label, help_text, min_val, max_val, default, step, key, is_percent=False, is_int=None):
    """
    רכיב "stepper" אמיתי - כפתורי +/- גדולים + תצוגת ערך בצבע אקסנט, בסגנון
    אפליקציות מובייל native. נבנה כווידג'ט משלו (לא CSS על st.slider) כי
    st.slider הוא פיזית סרגל-גרירה יחיד - אין דרך "לעצב" אותו לשני כפתורים
    נפרדים; ניסיון קודם לעשות זאת ב-CSS גרם פעמיים לחפיפת ערכים (הבאג שכבר
    תיקנו ע"י חזרה לברירת המחדל של Streamlit לסליידרים הרגילים).

    מנהל את הערך תחת st.session_state[key] ומחזיר את הערך הנוכחי.
    """
    if key not in st.session_state:
        st.session_state[key] = default

    current = st.session_state[key]
    is_int_val = is_int if is_int is not None else isinstance(step, int)
    value_display = f"{int(current)}%" if is_percent else (f"{int(current)}" if is_int_val else f"{current:.1f}")

    t = get_theme()
    card_html = f"""
    <div style="background:{t['panel']}; border:1px solid {t['border']}; border-radius:16px;
                padding:12px 16px 6px 16px; margin-bottom:6px;">
        <div style="display:flex; justify-content:space-between; align-items:flex-start;">
            <div>
                <div style="font-weight:700; font-size:14.5px; color:{t['text_main']};">{label}</div>
                <div style="font-size:11.5px; color:{t['text_muted']}; margin-top:2px;">{help_text}</div>
            </div>
            <div style="font-size:22px; font-weight:800; color:{ACCENT}; white-space:nowrap; margin-inline-start:10px;">{value_display}</div>
        </div>
    </div>
    """
    ...
    with c1:
        if st.button("−", key=f"{key}_minus", use_container_width=True):
            st.session_state[key] = round(max(min_val, current - step), 4)
            st.rerun()
    with c2:
        pct = (current - min_val) / (max_val - min_val) if max_val > min_val else 0.0
        st.progress(max(0.0, min(1.0, pct)))
        lo_disp = f"{min_val}{'%' if is_percent else ''}"
        hi_disp = f"{max_val}{'%' if is_percent else ''}"
        range_html = f'<div style="display:flex; justify-content:space-between; font-size:11px; color:{t["text_muted"]}; margin-top:-6px;"><span>{lo_disp}</span><span>{hi_disp}</span></div>'
        st.markdown(range_html, unsafe_allow_html=True)
    with c3:
        if st.button("+", key=f"{key}_plus", use_container_width=True):
            st.session_state[key] = round(min(max_val, current + step), 4)
            st.rerun()
...
def render_settings_panel():
    ...
    try:
        from modules.finnhub_source import finnhub_status_badge, get_finnhub_api_key, fetch_index_constituents
        is_configured, label = finnhub_status_badge()
    except Exception:
        is_configured, label = False, "⚪ Finnhub לא זמין"
    (st.success if is_configured else st.info)(label)
    with st.expander("איך מגדירים מפתח Finnhub (גיבוי ל-yfinance)"):
        st.markdown(
            "1. אם עוד אין לך מפתח - תירשם בחינם ב-[finnhub.io](https://finnhub.io) ותעתיק את ה-API Key.\n"
            "2. תיכנס לדשבורד של האפליקציה ב-**share.streamlit.io** (לא ב-GitHub).\n"
            "3. תלחץ על שלוש הנקודות ⋮ ליד האפליקציה שלך → **Settings** → **Secrets**.\n"
            "4. תדביק שורה בפורמט הזה בדיוק, ותשמור:\n"
        )
        st.code('FINNHUB_API_KEY = "המפתח_שלך_כאן"', language="toml")
        st.caption(
            "אחרי השמירה האפליקציה תעשה Reboot אוטומטי. Finnhub נכנס לפעולה רק כש-yfinance נכשל "
            "אחרי כמה ניסיונות (בעיקר בסריקות גדולות) - הוא לא מחליף את yfinance, רק גיבוי. "
            "טייר החינמי מוגבל לשנה אחת של נתונים לכל בקשה, אז לתקופות ארוכות יותר (24 חודשים, 5 שנים) "
            "האפליקציה מפצלת את הבקשה למספר קריאות."
        )

    if is_configured:
        with st.expander("🧪 בדיקה: האם רשימת מדדים (S&P 500 וכו') זמינה בחשבון שלך?"):
            st.caption(
                "יש דיווחים שה-endpoint הזה עבר לטייר בתשלום ב-Finnhub - הבדיקה כאן רצה מול "
                "השרת שלך בזמן אמת עם המפתח שהגדרת, כדי לדעת בוודאות בלי לנחש."
            )
            if st.button("הרץ בדיקה", key="test_fh_indices"):
                api_key = get_finnhub_api_key()
                status, payload = fetch_index_constituents("^GSPC", api_key)
                if status == 200 and isinstance(payload, dict) and payload.get("constituents"):
                    n = len(payload["constituents"])
                    st.success(f"✅ עובד! נמצאו {n} מניות ב-S&P 500. אפשר לבנות על זה טעינה אוטומטית.")
                elif status in (401, 403):
                    st.error(f"❌ סטטוס {status} - זה בדיוק הדיווח שראיתי: ה-endpoint הזה דורש טייר בתשלום "
                             f"בחשבון שלך. נמשיך עם רשימת כל השוק האמריקאי (`/stock/symbol`) במקום.")
                else:
                    st.warning(f"⚠️ תגובה לא צפויה (סטטוס {status}): {str(payload)[:300]}")
