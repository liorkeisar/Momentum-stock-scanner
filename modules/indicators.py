...
# ============================
# סיווג שלב Wyckoff (Accumulation / Markup / Distribution / Markdown)
# ============================
# ⚠️ הערה מתודולוגית חשובה: זהו סיווג היוריסטי מבוסס-כללים (rule-based) מתוך
# האינדיקטורים שכבר קיימים במנוע (SMA200 slope, Absorption, Sideways, מיקום
# בטווח 52 שבועות) - הוא *לא* זיהוי אמיתי של תבניות Wyckoff הקלאסיות (Spring,
# Sign of Strength, Last Point of Support וכו') שדורש בדרך כלל שיפוט מומחה על
# גבי הגרף. יש להתייחס לתיוג כאן כאינדיקציה כללית וגסה לשלב הסביר ביותר, לא
# כקביעה מוסמכת.

def compute_wyckoff_phase(df, res=None):
    """
    מסווג את המניה לאחד מ-4 שלבי Wyckoff, על בסיס:
    - מגמת-על (SMA50/SMA200 והשיפוע שלו) -> Markup / Markdown כשיש מגמה ברורה
    - תנועה "הצידה" (SidewaysSlope נמוך) + מיקום בטווח 52 שבועות -> Accumulation
      (קרוב לשפל) לעומת Distribution (קרוב לשיא)
    """
    try:
        close_now = safe_last(df["Close"])
        sma50 = safe_last(df["SMA50"]) if "SMA50" in df.columns else np.nan
        sma200 = safe_last(df["SMA200"]) if "SMA200" in df.columns else np.nan
        sma200_slope = safe_last(df["SMA200_slope"]) if "SMA200_slope" in df.columns else np.nan
        sideways_slope = safe_last(df["SidewaysSlope"]) if "SidewaysSlope" in df.columns else np.nan
        absorption = safe_last(df["AbsorptionScore"]) if "AbsorptionScore" in df.columns else np.nan
        updown_vol = safe_last(df["UpDownVolRatio"]) if "UpDownVolRatio" in df.columns else np.nan
        high52 = safe_last(df["High52W"]) if "High52W" in df.columns else np.nan
        low52 = safe_last(df["Low52W"]) if "Low52W" in df.columns else np.nan

        range_pos = 0.5
        if not is_bad(high52) and not is_bad(low52) and high52 > low52 and not is_bad(close_now):
            range_pos = max(0.0, min(1.0, (close_now - low52) / (high52 - low52)))

        is_uptrend = (not is_bad(close_now) and not is_bad(sma50) and not is_bad(sma200)
                      and close_now > sma50 > sma200 and (is_bad(sma200_slope) or sma200_slope > 0))
        is_downtrend = (not is_bad(close_now) and not is_bad(sma50) and not is_bad(sma200)
                        and close_now < sma50 < sma200 and (is_bad(sma200_slope) or sma200_slope < 0))
        is_sideways = not is_bad(sideways_slope) and abs(sideways_slope) < 1.0

        if is_uptrend and not is_sideways:
            phase = "markup"
        elif is_downtrend and not is_sideways:
            phase = "markdown"
        elif range_pos < 0.45:
            phase = "accumulation"
        elif range_pos > 0.55:
            ...
            phase = "markup" if is_uptrend else ("markdown" if is_downtrend else "accumulation")

        meta = {
            "accumulation": ("🟢", "Accumulation (צבירה)",
                              "טווח מסחר לאחר ירידה, קרוב לשפל 52 השבועות - שלב אפשרי של צבירה שקטה."),
            "markup": ("🚀", "Markup (עלייה)",
                       "מגמת-על עולה מבוססת (מחיר מעל SMA50 מעל SMA200 עולה) - שלב של ביקוש דומיננטי."),
            "distribution": ("🟠", "Distribution (פיזור)",
                              "טווח מסחר לאחר עלייה, קרוב לשיא 52 השבועות - ייתכן שלב של פיזור על ידי בעלי עניין."),
            "markdown": ("🔴", "Markdown (ירידה)",
                         "מגמת-על יורדת (מחיר מתחת ל-SMA50 מתחת ל-SMA200 יורד) - שלב של היצע דומיננטי."),
        }
        icon, label, desc = meta[phase]
        return {"phase": phase, "icon": icon, "label": label, "description": desc, "range_position": range_pos}
    except Exception:
        return {"phase": "unknown", "icon": "⚪", "label": "לא ידוע", "description": "לא ניתן לסווג שלב", "range_position": 0.5}
