...
def fetch_us_common_stocks(api_key=None, timeout=20):
    """
    שולף את רשימת כל המניות ה"רגילות" (Common Stock) הנסחרות בבורסות ארה"ב
    דרך Finnhub - חלופה חינמית ל-Indices Constituents (שנעול בטייר בתשלום).
    זו לא רשימת S&P 500 ספציפית - זה כל השוק (כמה אלפי טיקרים), כולל המון
    חברות זעירות/לא נזילות. סינוני הנזילות/מחיר שכבר קיימים בסורק אמורים
    לסנן את רובן בפועל. Cache ל-24 שעות כי הרשימה משתנה לעיתים נדירות.
    """
    api_key = api_key or get_finnhub_api_key()
    if not api_key:
        return []
    try:
        r = requests.get(
            "https://finnhub.io/api/v1/stock/symbol",
            params={"exchange": "US", "token": api_key}, timeout=timeout
        )
        if r.status_code != 200:
            return []
        data = r.json()
        ...
        tickers = [
            ...
            and "." not in row["symbol"] and "-" not in row["symbol"]
        ]
        seen = set()
        unique = []
        for t in tickers:
