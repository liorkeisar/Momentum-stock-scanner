...
from modules.finnhub_source import fetch_finnhub_history, get_finnhub_api_key
...
def load_history(ticker, period="12mo"):
    ...
    אם yfinance עדיין נכשל אחרי כל הניסיונות, ומפתח Finnhub מוגדר ב-Secrets -
    מנסה את Finnhub כגיבוי אחרון לפני שמוותרים לגמרי. yfinance נשאר תמיד
    המקור הראשי; Finnhub נכנס לפעולה רק בכישלון מוחלט.
    """
    import time
    last_err = None
    ...
    if get_finnhub_api_key():
        try:
            df_fh = fetch_finnhub_history(ticker, period=period)
    ...
    for name, ticker in indices.items():
        try:
            ...
            if len(hist) >= 2:
                ...
                # הגנה מפני NaN: yfinance מחזיר לפעמים שורה אחרונה חלקית/NaN
                # (במיוחד סמוך לפתיחת המסחר) - בלי הבדיקה הזו זה היה מוצג
                # פשוט כטקסט "nan"/"nan%" בכרטיס, כמו שקרה בפועל.
                if is_bad(last) or is_bad(prev):
                    continue
    ...
    if st.button("טען חדשות + המלצות אנליסטים", key=f"news_btn_{ticker}"):
        ...
        if analyst.get("error") and not analyst.get("recs") and not analyst.get("targets"):
            ...
            if recs and sum(recs.values()) > 0:
                ...
                buy_n = recs["strongBuy"] + recs["buy"]
                hold_n = recs["hold"]
                sell_n = recs["sell"] + recs["strongSell"]
                buy_pct = (buy_n / total) * 100
                hold_pct = (hold_n / total) * 100
                sell_pct = max(0.0, 100 - buy_pct - hold_pct)  # שארית, מונע פערי עיגול
                bar_html = f"""
                <div style="display:flex; height:14px; border-radius:8px; overflow:hidden; margin-bottom:10px; background:{t['border']};">
                    <div style="width:{buy_pct:.1f}%; background:{BUY_COLOR};"></div>
                    <div style="width:{hold_pct:.1f}%; background:{ACCENT};"></div>
                    <div style="width:{sell_pct:.1f}%; background:{SELL_COLOR};"></div>
                </div>
                <div style="display:flex; justify-content:space-between; font-size:12.5px; font-weight:700;">
                    <span style="color:{BUY_COLOR};">קנייה ({buy_n})</span>
                    <span style="color:{ACCENT};">החזקה ({hold_n})</span>
                    <span style="color:{SELL_COLOR};">מכירה ({sell_n})</span>
                </div>
                """
                st.markdown("".join(line.strip() for line in bar_html.split("\n")), unsafe_allow_html=True)
            else:
