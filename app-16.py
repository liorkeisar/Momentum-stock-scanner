...
from modules.styles import (
    ...
    render_top_nav, render_settings_panel,
...
if _active == "scanner":
    ...
    with st.sidebar.expander("🎯 סינון ציון בסיסי", expanded=True):
        ...
        min_confidence = st.slider(
            "ביטחון מינימלי (%):", 0, 100, 45,
            help="אחוז הרכיבים בציון שהגיעו לרף 'חזק' (70+). ברירת מחדל 45% מוודאת שהציון נתמך "
                 "על ידי כמה רכיבים ולא רכיב יחיד שדחף אותו."
        )
    ...
    with st.sidebar.expander("🧪 סינוני איכות מתקדמים", expanded=False):
        ...
        rsi_range = st.slider(
            "טווח RSI:", 0, 100, (35, 75),
            help="ברירת מחדל 35-75: טווח רחב יותר שממעט לצמצם יתר על המידה. 45-65 (צר יותר) מומלץ "
                 "כסינון נוסף ידני אם יש יותר מדי תוצאות, לא כברירת מחדל."
        )
        rvol_min = st.number_input(
            "נפח יחסי מינימלי (RVOL):", min_value=0.0, max_value=10.0, value=0.8, step=0.1,
            help="ברירת מחדל 0.8: לא דורש נפח חריג כרגע (זה מגיע בפריצה עצמה), רק מוודא שאין נטישת מסחר. 0 = ללא סינון."
        )
        atr_pct_range = st.slider(
            "טווח תנודתיות (ATR% מהמחיר):", 0.0, 15.0, (2.0, 6.0), step=0.5,
            help="ברירת מחדל 2%-6%: מסנן גם מניות שקטות מדי (אין אנרגיה לתנועה) וגם תנודתיות מסוכנת."
        )
    ...
    with st.sidebar.expander("⚙️ הגדרות סריקה", expanded=False):
        ...
        min_dollar_vol = st.number_input(
            ...
            help="ברירת מחדל $5M: נזילות אמיתית מונעת false signals ממניות שקטות מדי. "
                 "מניות עם מחזור מסחר דולרי (מחיר × נפח ממוצע) נמוך מהסף יסוננו."
        )
