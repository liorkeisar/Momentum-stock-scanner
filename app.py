...
def compute_features_for_ml(df, window=20):
    """
    מחזיר DataFrame של תכונות עבור חלונות גלילה:
    כל שורה = חלון של 'window' ימים, עם תכונות ממוצעות/אחרונות.
    """
    rows = []
    for end in range(window, len(df)-5):  # נשאיר 5 ימים קדימה לתווית
        ...
        # label: האם בתוך 5 ימים הקרובים תהיה סגירה מעל שיא החלון (breakout)
        future = df.iloc[end:end+5]
        label = 0
        if not future.empty:
            if future["Close"].max() > w["High"].max() * 1.005:  # פריצה קלה מעל השיא
                label = 1
        feat["label"] = label
        rows.append(feat)
...
def train_logistic_model(df):
    """
    מאמן מודל לוגיסטי על נתוני המניה עצמה (חלונות היסטוריים).
    מחזיר מודל או None אם לא ניתן לאמן.
    """
    try:
        feats = compute_features_for_ml(df, window=20)
        feats = feats.dropna()
        if len(feats) < 30 or feats['label'].sum() < 5:
            return None  # לא מספיק נתונים
...
def logistic_predict_probability(model, df):
    """
    מחזיר הסתברות לפי המודל; אם אין מודל — מחזיר None.
    """
    try:
        feats = compute_features_for_ml(df, window=20)
        if feats.empty:
...
def statistical_similarity_prediction(df, tolerance=0.15, lookahead=5):
    """
    מחפש חלונות היסטוריים דומים לפי כמה מדדים ומחשב אחוז הצלחה לפריצה בתוך lookahead ימים.
    החזרת dict עם count, successes, rate.
    """
    try:
        feats = compute_features_for_ml(df, window=20)
        if feats.empty:
            return {"count":0, "successes":0, "rate":0.0}
        # השתמש ב־compression (std20), rvol, trend proxy (ema20_ema50)
        target = feats.dropna().iloc[-1]
        candidates = feats.dropna().iloc[:-1]  # עבר
        if candidates.empty:
            return {"count":0, "successes":0, "rate":0.0}
        def similar(row):
            # יחסית לטווחים: בדיקה פשוטה של הבדלים יחסיים
            try:
...
def pattern_detection_vcp_like(df):
    """
    זיהוי פשוט של תבנית VCP-like: רצף של שיאים יורדים ועמקים עולים בתוך חלון 30 יום,
    וירידה ב־STD20 (דחיסה). מחזיר True/False ותיאור.
    """
    try:
        window = 30
        if len(df) < window:
        ...
        highs = w["High"].values
        lows = w["Low"].values
        # בדוק אם יש סדרת lower highs בשלושה מקטעים
        ...
        std_trend = np.polyfit(range(len(w["Close"].rolling(10).std().dropna())), w["Close"].rolling(10).std().dropna(), 1)[0] if len(w["Close"].rolling(10).std().dropna())>2 else 0
        ...
        if not tickers:
            ...
            if df_res.empty:
                ...
                if info:
                    ...
                    with colp1:
                        lookahead = st.selectbox("חלון חיזוי (ימים):", [3,5,7], index=1)
                        stat_tol = st.slider("סף דמיון סטטיסטי (אחוזי שונות):", 5, 50, 15)
                    with colp2:
                        if st.button("הרץ חיזוי עבור " + sel):
                            with st.spinner("מריץ חיזוי..."):
                                try:
                                    # 1) חיזוי סטטיסטי
                                    stat = statistical_similarity_prediction(info["df_tail"].copy().append(load_history(sel, period="12mo").iloc[:-120]) if True else info["df_tail"], tolerance=stat_tol/100.0, lookahead=lookahead)
                                    # 2) תבניות
                                    pat = pattern_detection_vcp_like(load_history(sel, period="12mo"))
                                    # 3) מודל לוגיסטי
                                    model = train_logistic_model(load_history(sel, period="24mo"))
                                    ml_prob = logistic_predict_probability(model, load_history(sel, period="24mo"))
