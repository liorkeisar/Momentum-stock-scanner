...
if _active == "scanner":
    ...
    with st.sidebar.expander("🧪 סינוני איכות מתקדמים", expanded=False):
        ...
        rsi_range = st.slider(
            "טווח RSI:", 0, 100, (20, 80),
            help="ברירת מחדל 20-80: טווח רחב יותר להשגת יותר תוצאות."
        )
        rvol_min = st.number_input(
            "נפח יחסי מינימלי (RVOL):", min_value=0.0, max_value=10.0, value=0.0, step=0.1,
            help="ברירת מחדל 0: ללא סינון נפח במצב התחלתי. אפשר להגביל בהמשך."
        )
        atr_pct_range = st.slider(
            "טווח תנודתיות (ATR% מהמחיר):", 0.0, 15.0, (0.5, 8.0), step=0.5,
            help="ברירת מחדל 0.5%-8%: מסנן גם מניות שקטות מדי וגם תנודתיות מסוכנת."
        )
    ...
    with st.sidebar.expander("⚙️ הגדרות סריקה", expanded=False):
        ...
        min_dollar_vol = st.number_input(
            ...
            value=500_000, step=500_000,
            help="ברירת מחדל $500K: נזילות בסיסית. נמוך יותר מ-$5M כדי להראות יותר מניות בתחילה."
        )
