...
def get_universe():
    ...
    if os.path.exists(filename):
        try:
            ...
            # מוודאים שיש עמודה בשם 'Symbol'
            if 'Symbol' in df.columns:
                return [str(t) for t in df['Symbol'].dropna().unique().tolist() 
                        if len(str(t)) < 6 and str(t).isalpha()]
        except Exception as e:
            st.error(f"שגיאה בקריאת הקובץ: {e}")
    return ["AAPL", "NVDA", "MSFT", "AMD", "TSLA"] # רשימת גיבוי
