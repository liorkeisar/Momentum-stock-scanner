...
st.set_page_config(layout="wide", page_title="Quantum Accumulation Terminal PRO", initial_sidebar_state="collapsed")

# --- CSS עיצוב פינטק פרימיום משודרג ---
st.markdown("""
    <style>
    ...
    /* קוביות נתונים מהירות (KPI Metrics) */
    .metric-card { background: #111522; border: 1px solid #1F2538; border-radius: 10px; padding: 12px; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.2); }
    ...
    .edu-title { color: #FCA311; font-size: 1.1rem; font-weight: 700; margin-bottom: 12px; display: flex; align-items: center; gap: 8px; }
    .edu-text { font-size: 0.88rem; color: #B5AEC4; line-height: 1.5; margin-bottom: 10px; }
    
    /* כפתורים ופקדים */
    .stButton>button { background: linear-gradient(180deg, #251F3D, #131124); color: #E6E1F3; border: 1px solid #362E54; border-radius: 12px; padding: 12px 24px; font-weight: 600; width: 100%; transition: all 0.2s; }
    .stButton>button:hover { border-color: #FCA311; color: #FCA311; background: #1A1530; }
    
    /* שיפור עיצוב הסליידרים ותיבות הקלט */
    div[data-testid="stMarkdownContainer"] h3 { font-size: 1.1rem; color: #FCA311; font-weight: 600; }
    </style>
...
def run_scanner(ticker):
    try:
        ...
        is_quiet = last_row['BB_Width'] <= 9.5  # הרחבה קלה מאוד לרוחב הרצועה למניעת פספוסים קלים
...
def draw_fixed_pro_chart(df, ticker):
    ...
    fig = make_subplots(rows=4, cols=1, shared_xaxes=True, row_heights=[0.52, 0.14, 0.17, 0.17], vertical_spacing=0.02)
    ...
    fig.update_layout(template="plotly_dark", paper_bgcolor="#0B0E14", plot_bgcolor="#0B0E14", height=540, margin=dict(l=5, r=40, t=5, b=5), showlegend=False, xaxis_rangeslider_visible=False, hovermode=False, dragmode=False)
...
def render_educational_card_and_calculator(df, unique_id=""):
    ...
    # שימוש בטאבים פנימיים לחלוקה נקייה של הממשק
    ...
    with sub_tab1:
        st.markdown(f"""
            <div class="edu-card">
                ...
                <div class="edu-text"><strong>1. מיצוי לחץ המוכרים:</strong> המניה הושמדה ב-<strong>{drop_val:.1f}%</strong> מהשיא השנתי שלה. כל הידיים החלשות והפאניקה כבר בחוץ, מה שמותיר רצפת מחיר יציבה.</div>
                <div class="edu-text"><strong>2. כיווץ אנרגיה:</strong> מדד התנודתיות (BBW) עומד על <strong>{bbw_val:.1f}%</strong>. המניה "ישנה" וזזה הצידה בתוך תעלה צרה מאוד, מה שמסמן קפיץ שנטען באנרגיה.</div>
                <div class="edu-text"><strong>3. כניסת מוסדיים שקטה:</strong> למרות שהמחיר דורך במקום, מדד זרימת הכסף (MFI) טיפס לרמה של <strong>{mfi_val:.1f}</strong>. לווייתנים אוספים סחורה מתחת לרדאר בלי להקפיץ את הנרות בגרף.</div>
            </div>
        """, unsafe_allow_html=True)
...
# --- ממשק משתמש ראשי ---
st.markdown('<h1 class="main-title">Quantum Terminal PRO</h1>', unsafe_allow_html=True)
st.markdown('<p class="sub-title">טרמינל מוסדי מתקדם לאיתור שלבי הצטברות ואיסוף סחורה חשאי (Accumulation Phase)</p>', unsafe_allow_html=True)

tabs_names = ["🔍 בדיקה ידנית", "NASDAQ א'", "NASDAQ ב'", "NASDAQ ג'", "S&P 500", "MIDCAP"]
...
for i, group_id in enumerate(sections_keys):
    with tabs[i + 1]:
        scan_clicked = st.button("הפעל סורק לווייתנים מוסדי", key=f"btn_{i}")
        
        if scan_clicked or st.session_state.get(f"accum_ready_{group_id}", False):
            if scan_clicked:
                with st.spinner("מריץ סורק אלגוריתמי על כל הקבוצה..."):
