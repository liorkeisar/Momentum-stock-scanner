...
from modules.styles import (
    ...
    render_top_nav, render_settings_panel, render_stepper,
...
from modules.finnhub_source import get_finnhub_api_key, fetch_us_common_stocks
...
if _active == "scanner":
    ...
    mode = st.sidebar.radio(
        "בחר מקור:",
        ["קובץ CSV בודד", "תיקיית CSV", "רשימת CSV בתיקייה הנוכחית", "הקלדה ידנית", "🌐 כל השוק האמריקאי (Finnhub)"]
    )
    ...
    elif mode == "🌐 כל השוק האמריקאי (Finnhub)":
        if not get_finnhub_api_key():
            st.sidebar.error("Finnhub לא מוגדר - הגדר מפתח ב-⚙️ הגדרות קודם.")
        else:
            with st.sidebar:
                with st.spinner("טוען רשימת מניות מ-Finnhub..."):
                    all_tickers = fetch_us_common_stocks()
            if all_tickers:
                st.sidebar.success(f"נטענו {len(all_tickers):,} מניות מהשוק האמריקאי")
                st.sidebar.caption(
                    "⚠️ זו לא רשימת S&P 500 - זה כל השוק (כולל המון חברות זעירות). "
                    "סינוני הנזילות/מחיר למטה עושים את רוב העבודה בבחירת מניות רלוונטיות. "
                    "מומלץ להעלות את 'מקסימום טיקרים לסריקה' בהגדרות הסריקה כדי לכסות יותר מהרשימה."
                )
                tickers = all_tickers
            else:
                st.sidebar.error("לא הצלחנו לטעון את הרשימה מ-Finnhub - נסה שוב או בדוק את המפתח.")
    ...
    with st.sidebar.expander("🎯 סינון ציון בסיסי", expanded=True):
        score_min_val = render_stepper(
            "טווח ציון - מינימום", "ציון נמוך מזה לא יוצג", 0, 100, 65, 5, "stepper_score_min", is_int=True
        )
        score_max_val = render_stepper(
            "טווח ציון - מקסימום", "96+ לרוב מרמז על קיצון/באג בנתונים", 0, 100, 95, 5, "stepper_score_max", is_int=True
        )
        min_score, max_score = min(score_min_val, score_max_val), max(score_min_val, score_max_val)

        min_confidence = render_stepper(
            "ביטחון מינימלי", "אחוז הרכיבים שהגיעו לרף 'חזק' (70+)", 0, 100, 45, 5, "stepper_confidence", is_percent=True, is_int=True
        )
    ...
    with st.sidebar.expander("🧪 סינוני איכות מתקדמים", expanded=False):
        ...
        require_stage2 = st.checkbox(
            ...
            help="הסינון המשמעותי ביותר לאיכות: מחייב Close > SMA150 > SMA200 עולה. "
                 "מומלץ להפעיל בנפרד לבדיקה - אבל ביחד עם שאר הסינונים למטה כברירת מחדל עלול לצמצם לאפס תוצאות."
        )
        rsi_min_val = render_stepper(
            "טווח RSI - מינימום", "מונע oversold קיצוני", 0, 100, 35, 5, "stepper_rsi_min", is_int=True
        )
        rsi_max_val = render_stepper(
            "טווח RSI - מקסימום", "מונע overbought קיצוני", 0, 100, 75, 5, "stepper_rsi_max", is_int=True
        )
        rsi_range = (min(rsi_min_val, rsi_max_val), max(rsi_min_val, rsi_max_val))

        rvol_min = render_stepper(
            "נפח יחסי מינימלי (RVOL)", "0 = ללא סינון", 0.0, 5.0, 0.8, 0.1, "stepper_rvol_min", is_int=False
        )

        atr_min_val = render_stepper(
            "תנודתיות (ATR%) - מינימום", "מסנן מניות שקטות מדי", 0.0, 15.0, 2.0, 0.5, "stepper_atr_min", is_percent=True, is_int=False
        )
        atr_max_val = render_stepper(
            "תנודתיות (ATR%) - מקסימום", "מסנן תנודתיות מסוכנת", 0.0, 15.0, 6.0, 0.5, "stepper_atr_max", is_percent=True, is_int=False
        )
        atr_pct_range = (min(atr_min_val, atr_max_val), max(atr_min_val, atr_max_val))

        price_min_val = render_stepper(
            "טווח מחיר ($) - מינימום", "הימנע מ-penny stocks", 0, 1000, 0, 5, "stepper_price_min", is_int=True
        )
        price_max_val = render_stepper(
            "טווח מחיר ($) - מקסימום", "הימנע ממניות יקרות מדי", 0, 1000, 1000, 5, "stepper_price_max", is_int=True
        )
        price_range = (min(price_min_val, price_max_val), max(price_min_val, price_max_val))
        phase_filter = st.multiselect(
            "שלב Wyckoff (סיווג היוריסטי - ראה הערה בדוח המפורט):",
            options=["accumulation", "markup", "distribution", "markdown"],
            default=["accumulation"],
            format_func=lambda p: {"accumulation": "🟢 Accumulation (צבירה)", "markup": "🚀 Markup (עלייה)",
                                    "distribution": "🟠 Distribution (פיזור)", "markdown": "🔴 Markdown (ירידה)"}[p],
            help="ברירת המחדל מציגה רק מניות בשלב 'צבירה' (Accumulation) - השלב הקלאסי של קדם-פריצה לפי "
                 "וייקוף (איסוף סחורה שקט לפני תנועה). הוסף שלבים נוספים אם תרצה להרחיב את הסריקה."
        )

    with st.sidebar.expander("⚙️ הגדרות סריקה", expanded=False):
        max_tickers = render_stepper(
            "מקסימום טיקרים לסריקה", "כמות טיקרים מקסימלית לבדיקה בסריקה אחת", 10, 1000, 200, 10, "stepper_max_tickers", is_int=True
        )
        min_dollar_vol = render_stepper(
            "מינימום מחזור מסחר יומי ($)", "נזילות אמיתית מונעת false signals ממניות שקטות מדי",
            0, 100_000_000, 5_000_000, 500_000, "stepper_min_dollar_vol", is_int=True
        )
    ...
    if run_scan:
        if not tickers:
            ...
            skipped_liquidity = []
            no_data_count = 0
            # מונה נפרד לכל סיבת סינון - כדי שאפשר יהיה לדעת בדיוק *למה* אין תוצאות,
            # במקום לראות "0 תוצאות" בלי שום פירוט (שאי אפשר להבחין ממנו בין בעיית
            # נתונים לבין סינון קפדני מדי).
            filtered_reasons = {
            ...
            for i, ticker in enumerate(tickers):
                ...
                try:
                    ...
                    if exclude_broken_out and res.get("already_broken_out"):
                        filtered_reasons["already_broken_out"] += 1
                        continue
                    if exclude_downtrend and res.get("hard_downtrend"):
                        filtered_reasons["hard_downtrend"] += 1
                        continue
                    if require_stage2 and not res.get("stage2_ok"):
                        filtered_reasons["stage2"] += 1
                        continue
            ...
            reason_labels = {
                "already_broken_out": "כבר פרצה משמעותית", "hard_downtrend": "מגמת-על יורדת",
                "stage2": "לא עומדת ב-Stage 2", "rsi": "RSI מחוץ לטווח", "rvol": "נפח יחסי נמוך מדי",
                "atr": "תנודתיות (ATR%) מחוץ לטווח", "confidence": "ביטחון נמוך מדי", "price_range": "מחוץ לטווח המחיר",
            }
            total_quality_filtered = sum(filtered_reasons.values())
            valid_data_count = total - no_data_count - len(skipped_liquidity)

            st.caption(
                f"📊 נסרקו {total} טיקרים · {no_data_count} ללא נתונים · {len(skipped_liquidity)} נזילות נמוכה · "
                f"{valid_data_count} עם נתונים תקינים · {len(results) - no_data_count} עברו את כל הסינונים"
            )
            if total_quality_filtered > 0:
                with st.expander(f"🔍 למה {total_quality_filtered} טיקרים עם נתונים תקינים לא עברו את הסינון? (לחץ לפירוט)"):
                    for key, count in sorted(filtered_reasons.items(), key=lambda x: -x[1]):
                        if count > 0:
                            st.caption(f"• {reason_labels[key]}: {count} טיקרים")
                    st.caption(
                        "💡 אם רוב הטיקרים נופלים על סיבה אחת ספציפית - זה סימן שהסינון הזה קפדני מדי "
                        "עבור תנאי השוק הנוכחיים. נסה להקל בו בסיידבר."
                    )
    ...
    if "scan_results" in st.session_state and st.session_state["scan_results"]:
        ...
        if phase_filter and len(phase_filter) < 4:
            def _phase_ok(t):
                info = details.get(t)
                if not info or not info.get("phase"):
                    return True  # אין מידע על שלב - לא מסננים (שמרני)
                return info["phase"]["phase"] in phase_filter
            df_res = df_res[df_res["Ticker"].apply(_phase_ok)]
        ...
        if df_res.empty:
            ...
            if view_mode == "🗂️ פיד":
                for _, row in df_res.iterrows():
                    ...
                    info = details.get(t)
                    if info:
                        render_stock_card(t, info["res"], info["df_tail"], phase=info.get("phase"))
                    else:
            ...
            if info:
                ...
                if phase and phase.get("phase") != "unknown":
                    st.markdown(f"**שלב Wyckoff (היוריסטי):** {phase['icon']} {phase['label']}")
                    st.caption(f"{phase['description']} ⚠️ זהו סיווג היוריסטי מבוסס-כללים, לא זיהוי תבניות Wyckoff קלאסיות (Spring/SOS/LPS) שדורש שיפוט מומחה על הגרף.")

                st.caption(f"💪 חוזק הדפוס: {res['confidence']}%")
                st.progress(min(100, max(0, res["confidence"])) / 100)
